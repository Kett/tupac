...
function sendMessage($msgId, $msgData) {
...
function httpSocketConnection($host, $method, $path, $data) {
	...
	if ($GLOBALS['tupacrcData']['%PROXY_NAME%'][0] && $GLOBALS['tupacrcData']['%PROXY_PORT%'][0]) {
		$path='http://'.$host.$path;
		$host=$GLOBALS['tupacrcData']['%PROXY_NAME%'][0];
		$port=$GLOBALS['tupacrcData']['%PROXY_PORT%'][0];
	}
...
function setProxy($proxy) {
	switch ($proxy) {
		...
		default:
			...
			$fh=fopen($GLOBALS['tupacrc'],'w');
			fwrite($fh, "%PROXY_NAME%\n$proxy[0]\n%PROXY_PORT%\n$proxy[1]\n");
			fclose($fh);
			break;
	}
}
